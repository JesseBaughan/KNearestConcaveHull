...
namespace Clustering
{
    ...
    */
    ...
    /*
    bool Intersects(pdd intersectPoint, pdd A, pdd B, pdd C, pdd D)
    {
        bool xTrue = intersectPoint.first >= min(A.first, C.first) && intersectPoint.second <= max(A.)
    }
    */
    ...
    bool LineIntersects(const std::vector<lat_lon_coord>& points)
    {
        ...
        {
            pdd lineOneFirstPoint(points[i].Lat, points[i].Lon);
            pdd lineOneSecondPoint(points[i + 1].Lat, points[i + 1].Lon);
            for(int j = 0; j < points.size() - 1; j++)
            {
                ...
                {
                    pdd lineTwoFirstPoint(points[j].Lat, points[j].Lon);
                    pdd lineTwoSecondPoint(points[j + 1].Lat, points[j + 1].Lon);
                    pdd intersectPoint = lineLineIntersection(lineOneFirstPoint, lineOneSecondPoint, 
